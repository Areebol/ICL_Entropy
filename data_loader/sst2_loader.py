...
class SST2Loader():
    def __init__(self) -> None:
        self.name = 'sst2_sentiment_classification'
        ...
        self.test_data = [line for line in dataset['validation']]
        
        self.ICL0 = [f"Review:{d['sentence'].strip().replace("\\n", " ")}\nSentiment:{self.label[d['label']]}" for d in self.train_data if d['label']==0]
        self.ICL1 = [f"Review:{d['sentence'].strip().replace("\\n", " ")}\nSentiment:{self.label[d['label']]}" for d in self.train_data if d['label']==1]
